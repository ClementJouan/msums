	...
	for(SET_ITER set1=start1, set2=start2;
		...
		for(size_t s2=0; s2<set2->size(); s2++){
			if (s1 != s2 & s1 == 0){
				...
				//cout << "s1: " << s1 << " " << "(*set1)[s1]: " << (*set1)[s1] << " " << "s2: " << s2 << " " << "(*set2)[s2]: " << (*set2)[s2] << " " << "count: " << count <<  "\n";
				}
			}
			}
	return count;
...
double sum_pair_mini(
	...
	for(SET_ITER set1=start1, set2=start2;
		...
		for(size_t s2=0; s2<set2->size(); s2++){
			if (s1 != s2){
				mini = min((*set1)[s1],(*set2)[s2]);
				vec.insert(vec.end(), mini);
				}
	...
	auto results = *std::min_element(vec.begin(), vec.end());
	vec.clear();
	return results;
	}
...
struct PolyL
	...
	void compute(
		...
		//cout << "sfout: " << sfout << " " << "sxA: " << sxA << " " << "sxB: " << sxB << " " << "sxAfB: " << sxAfB << " " << "sxBfA: " << sxBfA << " " << "sfA: " << sfA << " " << "sfB: " << sfB << "\n";
